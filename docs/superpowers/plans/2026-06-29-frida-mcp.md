...
> Note: session/note/event tools route through `manager.store`. The test's `FakeManager` sets `self.store = self` and defines the store-side methods it needs, so the same wrappers work against both the real `ProjectStore` and the fake — no production branching for tests.
